...
export async function assistantReply(admin: SupabaseClient, userId: string, userMessage: string, apiKey: string, door = "whatsapp"): Promise<{ reply: string; created: any[]; threadId?: string }> {
  ...
  async function doMoveUpdate(input: any): Promise<string> {
    ...
    try {
      const { data: cs } = await admin.from("comment").select("id,text,created_at").eq("card_id", from.id).eq("by_name", "buno").order("created_at", { ascending: false }).limit(10);
      ...
      await admin.from("comment").delete().eq("id", pick.id);
      changed.push(from.id);
      if (to) { await admin.from("comment").insert({ card_id: to.id, by_name: "buno", text: pick.text }); changed.push(to.id); return `העברתי את העדכון מ"${from.title}" ל"${to.title}".`; }
