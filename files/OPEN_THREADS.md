...
- **Today:** a boolean `viewer` flag lets the owner **simulate** a non-owner's screen (`DemoTag "תצוגת לקוח · הדגמה"`).
...
- **Today:** `notifSeen` and `lastReset` (daily routine reset) live in browser storage → they're **per-device**.
...
- **Today:** attachments are dataURLs in IndexedDB (local); the `attachments` bucket exists but upload wiring isn't connected; storage policies (`0005_storage_policies.sql`) pending run.
